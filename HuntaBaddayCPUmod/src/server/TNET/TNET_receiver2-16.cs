...
namespace HuntaBaddayCPUmod {
    public class TNET_Recv2_16 : LogicComponent {
        const int pin_bus = 0;
        ...
        const int pin_input = 21;
        const int pin_reset = 22;
        const int pin_empty = 16;
            ...
            if(getPin(pin_read) && getPin(pin_rs) && getPin(pin_enable)){
                ...
                if(!hasRead){
                    if(output_position != output_length){
                        if(getPin(bit_mode)){
                            bool empty = false;
                            ushort tmp = (ushort)(output_buffer[output_position++]);
                            if(output_position != output_length){
                                tmp |= (ushort)(output_buffer[output_position++]<<8);
                            } else {
                                empty = true;
                            }
                            writeBus(tmp);
                            setPin(pin_empty, empty);
                        } else {
                            writeBus((ushort)output_buffer[output_position++]);
                            setPin(pin_empty, false);
                        }
                    } else {
                        writeBus(0);
